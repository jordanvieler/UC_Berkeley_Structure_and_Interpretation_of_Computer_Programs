...
def about(topic):
    ...
    def select(paragraph):
        ...
        word_list = split(clean_paragraph)
        for t in topic:
            if t in word_list:
                return True
        return False
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    diff_list = [diff_function(user_word, v, limit) for v in valid_words]
    diff_iter = zip(diff_list, valid_words)
    diff_min = min(diff_iter, key=lambda x: x[0])
    if diff_min[0] <= limit:
        return diff_min[1]
    return user_word
